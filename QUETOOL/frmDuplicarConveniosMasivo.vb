...
Public Class frmDuplicarConveniosMasivo
...
#End Region
    ...
    Private Sub frmDuplicarConveniosMasivo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Call BuscarMedios()
...
#Region "Metodos"
    ...
    Private Function AplicarFormatoGrilla()
        ...
        Listas.OcultarColumnas(Me.uigGrillaConvenios, "ID_SOPORTE")
    ...
    Private Function BlanquearColumnaSel(ByVal DtResultado As DataTable)

        Dim DrResultado As DataRow

        For Each DrResultado In DtResultado.Rows
            DrResultado("Sel") = False
        Next
...
#End Region
    ...
    Private Sub cmdBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdBuscar.Click
        ...
        DtResultado = ObConvevios.ObtenerReporteConvenioDuplicar(Cliente, Medio, Soporte, CDate(Me.dtpFVigencia.Value))

        DtResultado = FiltrarXTipoSoporte(DtResultado)

        If DtResultado.Rows.Count > 0 Then
            Me.txtregistrosCantidad.Text = DtResultado.Rows.Count.ToString()
            DtResultado.Columns.Add("Sel", GetType(Boolean))
            DtResultado.Columns.Item("Sel").SetOrdinal(0)
            DtResultado.Columns.Item("ID_CONVENIO").SetOrdinal(1)
            DtResultado = BlanquearColumnaSel(DtResultado)
    ...
    Private Function QuitarDuplicados(ByVal Dt As DataTable) As DataTable

        Dim dtFinal As DataTable
        Dim dtUnicoIdConvenio As New DataTable
        Dim Dr As DataRow
        Dim RowFinal() As DataRow
        Dim FilaNueva As DataRow

        With dtUnicoIdConvenio.Columns
            .Add("ID_CONVENIO", GetType(Integer))
        End With

        dtFinal = Dt.Clone
        dtUnicoIdConvenio = Dt.DefaultView.ToTable(True, "ID_CONVENIO")

        For Each Dr In dtUnicoIdConvenio.Rows
            FilaNueva = dtFinal.NewRow
            RowFinal = Dt.Select("id_convenio = " & Dr("id_convenio"))
            dtFinal.ImportRow(RowFinal(0))
        Next
    ...
    Private Sub cmdDuplicar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDuplicar.Click
        ...
        Dim DrConvenios As DataRow
        Dim ObjConvenios As New Convenios
        Dim idmedio, id_soporte, id_convenio, cod_convenio, id_cliente As Integer
        Dim FechaFin As Date = Me.DtpFInicio.Value
        Dim DtResultado As DataTable

        DtConvenios = uigGrillaConvenios.DataSource

        If DtConvenios.Rows.Count > 0 Then
            For Each DrConvenios In DtConvenios.Rows
                If DrConvenios("sel") = True Then
                    idmedio = DrConvenios("ID_MEDIO")
                    id_soporte = DrConvenios("ID_SOPORTE")
                    id_convenio = DrConvenios("ID_CONVENIO")
                    cod_convenio = DrConvenios("cod_convenio")
                    id_cliente = DrConvenios("id_cliente")
                    ObjConvenios.InsertarDuplicado(cod_convenio, Me.DtpFInicio.Value, DtFFin.Value, Activo.Login.ToString(), idmedio, id_soporte, id_cliente, id_convenio)
                End If
            Next


            FechaFin = FechaFin.AddDays(-1) 'Resta 1 días
            ...
            For Each DrConvenios In DtConvenios.Rows
                If DrConvenios("sel") = True Then
                    ...
                    cod_convenio = DrConvenios("cod_convenio")
                    id_cliente = DrConvenios("id_cliente")
                    ObjConvenios.ActualizarFechaFin(cod_convenio, FechaFin, Activo.Login.ToString(), idmedio, id_soporte, id_cliente)
                End If
            Next

            Me.uigGrillaConvenios.DataSource = Nothing

            MsgBox("Se generaron los convenios solicitados convenios", MsgBoxStyle.Information, "Everest")

        Else
            MsgBox("No se encontraron registros para generar convenios", MsgBoxStyle.Information, "Everest")
        End If
